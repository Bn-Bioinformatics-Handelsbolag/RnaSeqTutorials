...
tutorial:
  id: "org.bnbio.tutorials.08_non_model_organism_differential_gene_expression"
  version: 0.0.1
output:
  learnr::tutorial:
    progressive: true
...
# to run locally replace tutorials/08_non_model_organism_differential_gene_expression/www with inst/tutorials/08_non_model_organism_differential_gene_expression/www

# to run as an app replace tutorials/08_non_model_organism_differential_gene_expression/www with www
...
# ---
# data
# txi <- readRDS(here("tutorials/08_non_model_organism_differential_gene_expression/www/tximport.rds"))
# counts <- round(txi$counts)
# sample_file <- here("tutorials/08_non_model_organism_differential_gene_expression/www/samples_table.tsv")
# samples <- read_tsv(sample_file,col_types=c("cff"))
# dds <- DESeqDataSetFromTximport(
#   txi=txi,
#   colData = samples,
#   design = ~ stage * tissue)
# saveRDS(dds,file=here("tutorials/08_non_model_organism_differential_gene_expression/www/dds-pre.rds"))
# dds <- DESeq(dds)
# saveRDS(dds,file=here("tutorials/08_non_model_organism_differential_gene_expression/www/dds-post.rds"))
# ZE_vs_FMG_at_B4 <- results(dds,name="tissue_ZE_vs_FMG")
# saveRDS(ZE_vs_FMG_at_B4,file=here("tutorials/08_non_model_organism_differential_gene_expression/www/ZE_vs_FMG_at_B4.rds"))
# ZE_vs_FMG_at_B8 <- results(dds,list(c("tissue_ZE_vs_FMG","stageB8.tissueZE")))
# saveRDS(ZE_vs_FMG_at_B8,file=here("tutorials/08_non_model_organism_differential_gene_expression/www/ZE_vs_FMG_at_B8.rds"))
```

## Introduction

In the previous tutorial, we saw that the data quality looks good, albeit one sample has failed and was undersequenced. In a separate [analysis](https://github.com/nicolasDelhomme/BioQAExample/tree/non-model), we substituted that sample for a replacement and redid the Biological QA. While the replciate did cluster well together, we observed a bias in the number of expressed genes between the tissues ZE and FMG, with the former expressing 10000 more.
We did not take any corrective measure during that second Exploratory Data Analysis (EDA), but suggested different approaches, such as using more stringent log2 fold-change cutoffs or investingating whether filtering genes deemed uninformative by DESeq2 would help.

The Principal Component Analysis (PCA) and the heat-map revealed that the first three dimensions were linked to the studies design variable: `stage` and `tissue`. Albeit the first component was linked to `tissue`, the heat-map revealed a stronger separation based on the `stage`.

This gives us some food for thoughts, as we need to decide how to best conduct the Differential Gene Expression (DGE) analysis, especially when looking at the `tissue` effect.
...
dds <- readRDS(here("tutorials/08_non_model_organism_differential_gene_expression/www/dds-pre.rds"))
```
...
dds <- readRDS(here("tutorials/08_non_model_organism_differential_gene_expression/www/dds-post.rds"))
```
...
quiz(caption="Where is the rest of the data?",
     question("Have you noticed that the quantile of expression started at 20%? Why?",
     ...
     question("As a follow up on the different number of genes expressed in ZE and FMG, is the independent filtering a good approach to remove noise in the assembly?",
              answer("YES",correct=TRUE,message="My opinion, but it is certainly worth a try. It would leaves us with about 34000 genes, pretty closed to what we would expect from the expression within a tissue in spruce."),
              answer("NO",message="Curious to hear why you think so."),
              answer("Worth a try",correct=TRUE),
...
Note the fourth gene has only missing values (it was never expressed), while all but the second and sixth have a missing `padj`. This is the telltale sign that these genes have been deemed uninformative.

We will revisit these results, but prior to that we need to learn about modelling in R, as the results we just observed were tests conducted on the model $expression = f(stage * tissue)$.
...
Why was that relevant? Imagine: $brain=gene\_expression$ and $body=tissue * stage$. Running `DESeq()` on our data will have done something highly similar to what we just did and `Chimpanzee` and `Human` would be our "candidate" differentially expressed genes!

## Results

Going back to our studies, the model we provided to `DESeq()` is $\tilde{ }stage*tissue$. `DESeq()` performed a DE analysis, conducted tests and applied a multiple testing correction as we have seen. But what did he perform the comparison on? Figure it out using the `resultsNames()` and `results()` function.
...
To look into the semantic of DE testing, and understand its meaning (which is non trivial), there is a nice application developed by Charlotte Soneson _et al._, in an `R` package called [ExploreModelMatrix](https://bioconductor.org/packages/release/bioc/html/ExploreModelMatrix.html). The package can be deployed as a `shiny` app or run standalone. 
...
**Note:** The default reference is devised by the alphabetic order of the variable, hence the reason why `control` is the default in this image and the tissue `FMG` in our data.
...
There are different ways of writing contrasts; what I have described above is the approach used by `DESeq2`. `edgeR` use model matrices and tend to rely on using no reference; _e.g._ instead of using `~tissue * stage`, one could create a single variable `condition` that would concatenate tissue and stage in a single factor (_i.e._ in our cases FMG_B4, ZE_B4, FMG_B8 and ZE_B8) and use a simpler model: `~0+condition`. Then one can write any contrast easily in math terms, _e.g._ looking at the example above (effect of the treatment in genotype B) would be written as $ZE\_B8 - (FMG\_B8 + ZE\_B4)$
...
Now that we know more about contrasts, we can extract them accordingly. The studies design (or rather the subset we are looking at) is about the developmental stages B4 and B8 of two embryonic tissue, the female megagametophyte (FMG) and the zygotie embryo (ZE). Our design is `stage * tissue` 
...
**Note:** If the default ordering is not what you want, you can use _e.g._ `dds$tissue <- relevel(dds$tissue,"ZE")` to make `ZE` the reference.
...
quiz(caption="Contrast by default",
     question("Which contrast is returned by default?",
              answer("Intercept"),
              answer("stage_B8_vs_B4"),
              answer("tissue_ZE_vs_FMG"),
              answer("stageB8.tissueZE",correct=TRUE),
              answer("The last one listed by resultsNames(), always",correct=TRUE),
              try_again=TRUE),
     question("What does that contrast represent?",
              answer("The average expression in ZE at stage B8 compared to 0",message="This would imply a model of the form `0 + ...`"),
              answer("The average expression in ZE at stage B8 compared to the baseline",message="Take a look at the `~tissue * stage` example in the previous chapter..."),
              answer("The average contribution to the expression level that is specifically attributable to ZE at stage B8",correct=TRUE),
              try_again=TRUE),
     question("How do we extract a similar information at stage `B4`?",
              answer("By looking at the `stage_B8_vs_B4` contrast",message="That would look at the stage effect in the FMG samples."),
              answer("By looking at the `tissue_ZE_vs_FMG` contrast",correct=TRUE),
...
```{r tissue-B4, exercise=TRUE, exercise.eval=TRUE, exercise.setup="dds-post"}
...
```{r tissue-B4-solution}
ZE_vs_FMG_at_B4 <- results(dds,name="tissue_ZE_vs_FMG")
```

```{r ZE_vs_FMG_at_B4, include=FALSE, eval=TRUE}
ZE_vs_FMG_at_B4 <- readRDS(file=here("tutorials/08_non_model_organism_differential_gene_expression/www/ZE_vs_FMG_at_B4.rds"))
```
...
write_tsv(ZE_vs_FMG_at_B4,file=here("analysis/ZE_vs_FMG_at_B4_DE-results.tsv"))
```

Now, wait! How do we know how many genes are differentially expressed? Good question, give a try at `summary()`.

```{r summary, exercise=TRUE, exercise.eval=TRUE, exercise.setup="ZE_vs_FMG_at_B4"}
...
```{r cutoffs, exercise=TRUE, exercise.eval=TRUE, exercise.setup="ZE_vs_FMG_at_B4"}
sum(!is.na(ZE_vs_FMG_at_B4$padj) & 
      ZE_vs_FMG_at_B4$padj <= 0.01 & 
      abs(ZE_vs_FMG_at_B4$log2FoldChange) >= 0.5)

```

```{r cutoffs-solution}
sum(!is.na(ZE_vs_FMG_at_B4$padj) & ZE_vs_FMG_at_B4$padj <= 0.01 & abs(ZE_vs_FMG_at_B4$log2FoldChange) >= 0.5)
res <- results(dds)
sum(!is.na(res$padj) & res$padj <= 0.01 & abs(res$log2FoldChange) >= 0.5)
```

And we could be even more stringent on the `log2FoldChange` value as we know that there is a bias in our data introduced by the different number of genes expressed in the two tissues.

#### Other contrasts

What if we are interested in comparing ZE and FMG at stage B8 (the reference being FMG at stage B4)? What about the developmental effect specifically in the ZE samples (the reference being in between the FMG samples)? None of the existing contrasts can give us that information. Here, we will need to use the `contrast=` argument of the `results()` function and that is where the previous section will become handy, as well as Charlotte's `ExploreModelMatrix`.
...
```{r ZE_vs_FMG_at_B8-quiz}
quiz(caption="Looking for Ze _vs._ FMG at B8",
     question("Which contrast can we use for that?",
              answer('c("tissue","ZE","FMG")'),
              answer('list(c("tissue_ZE_vs_FMG","stageB8.tissueZE"))',correct=TRUE),
              answer("c(0,0,1,1)",correct=TRUE)))
```

Question: Take a minute to reflect on how that is different from `stageB8.tissueZE` 

Answer: (B8 at ZE of expression the to contribution specific the, term interaction an is stageB8.tissueZE that is answer the) <-- read the answer from right to left.
...
ZE_vs_FMG_at_B8 <- results(dds,list(c("tissue_ZE_vs_FMG","stageB8.tissueZE")))
suppressPackageStartupMessages(source(here("tutorials/08_non_model_organism_differential_gene_expression/www/volcanoPlot.R")))
volcanoPlot(ZE_vs_FMG_at_B8)
```

```{r ZE_vs_FMG_at_B8, include=FALSE, eval=TRUE}
ZE_vs_FMG_at_B8 <- readRDS(file=here("tutorials/08_non_model_organism_differential_gene_expression/www/ZE_vs_FMG_at_B8.rds"))
```
...
In the Biological QA, we have used the `gplots` package `heatmap.2` function. That function follows the principles of base `R` and while being efficient, is not as easily customisable as implementations that are inspired from the `tidyverse`. Here we will use the `ComplexHeatmap` package `Heatmap()` and `HeatmapAnnotation()` functions to visualise the expression of the DE genes in the contrast we just ran. This is also a good sanity check as we should obtain a very blue and red chequered heat-map.

```{r heatmap, echo=TRUE, eval=TRUE}
suppressPackageStartupMessages(library(ComplexHeatmap))

row_sel <- !is.na(ZE_vs_FMG_at_B8$padj) & 
  ZE_vs_FMG_at_B8$padj <= 0.01 & 
  abs(ZE_vs_FMG_at_B8$log2FoldChange) >= 0.5

col_sel <- dds$stage=="B8"
...
```{r DE-counts, exercise=TRUE, exercise.eval=TRUE, exercise.setup="ZE_vs_FMG_at_B8"}
row_sel <- !is.na(ZE_vs_FMG_at_B8$padj) & 
  ZE_vs_FMG_at_B8$padj <= 0.01 & 
  abs(ZE_vs_FMG_at_B8$log2FoldChange) >= 0.5
...
```{r DE-counts-hint-3}
ggplot(tibble(y=...$baseMean),aes(y=y,x="ZE vs. FMG at B8")) + geom_boxplot() + scale_y_log10("average normalised counts")
# you could add another violin layer, give the boxplot a `notch`, and remove the x axis `x` label
```


```{r DE-counts-solution}
ggplot(tibble(y=ZE_vs_FMG_at_B8[row_sel,"baseMean"]),aes(y=y,x="ZE vs. FMG at B8")) + 
  geom_violin() + scale_y_log10("average normalised counts") + 
  scale_x_discrete(element_blank()) + geom_boxplot(notch=TRUE)
```

About half of DE genes is "expressed" below a 100 counts. This is not reassuring and further suggests that a stronger filtering of the input transcriptome would be beneficial.
...
We will start by exporting the results, so we can do the downstream analysis!

```{r export, echo=TRUE, eval=FALSE}
# complex
write_tsv(as.data.frame(ZE_vs_FMG_at_B8) %>% rownames_to_column("geneID"),
          file=here("analysis/ZE_vs_FMG_at_B8_full-model_DE-results.tsv"))
```
